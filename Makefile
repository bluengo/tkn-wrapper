...
define print_help
	...
	@echo -e "\tmake $(GRN)deploy-ocp-regular$(RST)        Deploy a regular OCP cluster at provided $(YLW)VERSION$(RST) in PSI"
	@echo -e "\tmake $(GRN)deploy-ocp-proxy$(RST)          Deploy a proxy OCP cluster at provided $(YLW)VERSION$(RST) in AWS"
	@echo -e "\tmake $(GRN)deploy-ocp-disconnected$(RST)   Deploy an air-gapped OCP cluster at provided $(YLW)VERSION$(RST) in AWS"
	@echo -e "\tmake $(RED)destroy-cluster$(RST)           Destroys existent cluster provided by $(BLU)NAME$(RST)"
	@echo -e "\nVariables:"
	@echo -e "\t$(YLW)VERSION$(RST)                        The OCP version of for the new cluster to be created"
...
deploy-ocp-regular:
	...
	@echo "Cluster version is $(OCP_VERSION)"
